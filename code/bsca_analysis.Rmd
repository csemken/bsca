...
if (exists('plot_path')) {
  knitr::opts_chunk$set(fig.path=plot_path, dev=c('postscript'))
}
```
...
We obtain the number of observations and prepare the data for regression.
...
Given these coefficients, we use the function `getOR` to obtain odds ratios for increasing the TV use from low to medium/high, and for increasing the electronic device use from 0 hours to various levels.  The function exponentiates the coefficient estimates and formats the result. The normalized electronic device use levels correspond to 
- 0.17: < 1 hour
- 0.33: 1 hour
- 0.50: 2 hours
- 0.67: 3 hours
...
```{r}
getOR(yrbs_coef, treat='Electronic Device Use', 
      treatvals=sort(na.omit(unique(yrbs$data$q82_nr))), digits=2)
```
...
The control variables that were omitted by @orben_association_2019 have important associations.

```{r}
getOR(yrbs_coef, treat='Male', treatvals=1, digits=2)
getOR(yrbs_coef, treat='Grade', treatvals=1, digits=2)
```


### MCS data

Next, we load the MCS dataset and run Bayesian model selection and averaging, analogously to the above analysis of the YRBS data. As described in the main manuscript, for illustration in our analysis we considered 4 outcome variables, 5 treatments and 14 potential control variables (results for other outcome variables are shown in Fig. S1). We display the names of these variables, which have been stored in the `mcs` workspace, as well as the number of observations.
...
The code below runs BMA for all 5 outcome variables. 
...
for (idy in 1:length(yvars)) {
  ...
  # BMA
  message(yname)
  mcs_ms[[yname]] = mombf:::modelSelection(
    y ~ ., data=datareg, includevars=1, family="binomial", 
    priorDelta=modelbbprior(1,1), priorCoef=bicprior(), priorGroup=bicprior(),
    niter=15000  # make sure we have at least 100 models for all outcomes
  ); cat('\n')
...
```{r bsca-single-yrbs, fig.height=4, fig.width=7}
idx_fit = c(2:4) 
id_years = c(12:14)
...
```{r bsca-single-mcs, fig.height=5, fig.width=7}
...
```{r bsca-multiple, fig.height=4, fig.width=7}
source('functions.R')
...
We perform subgroup analyses for males using the continuous MCS outcomes, normalized to 1-10. After selecting the right data, we parameterize the variables as described in the paper. Interaction terms are included only if the corresponding main variable is included. We then perform BMA using `mombf:::modelSelection`.
...
idg="Male"
{
  ...
  x.reg = model.matrix(~ ., data[,x_vars])[, -1]
  colnames(x.reg) = x_names
}

ms_inter = list(); coef_inter = list()
for (idy in 1:length(y_cont)) {
  yname = names(y_cont)[idy]
  
  datareg = na.omit(data[c(y_cont[idy], x_vars, cvars)])
  names(datareg) = c('y', x_names, names(cvars))
  ...
  ms_inter[[yname]] = mombf:::modelSelection(
    d$y, d$x, includevars=1, priorDelta=modelbbprior(1,1), 
    priorCoef=bicprior(), priorGroup=bicprior(),
    groups=d$groups, constraints=d$constraints,
  )
...
We show the results in a multiple-outcome BSCA. 

```{r bsca-subgroup, fig.height=4, fig.width=7}
multi_bsca(
  coef_inter, ms=ms_inter, treatments=paste0("`", colnames(inter), "`"),
  extract.color=TRUE
) 
```

The interaction coefficients for self-reported depression are:

```{r}
print(coef_inter$`Depression (adolescent)`[paste0("`", colnames(inter), "`"), ])
```

The interaction coefficients for self-reported self-esteem are:

```{r}
print(coef_inter$`Self-esteem (adolescent)`[paste0("`", colnames(inter), "`"), ])
```


## Partial correlations

As mentioned, in Section 2.5 of the main text, one can obtain partial correlations from the results of a linear regression.
Here we show how to implement this strategy using the MCS data.

We first run a linear regression, see also the robustness section below.
...
for (idy in 1:length(y_cont)) {
  ...
  ms_lin[[yname]] = mombf:::modelSelection(
    y ~ ., data=datareg, includevars=1, priorDelta=modelbbprior(1,1), 
    priorCoef=bicprior(), priorGroup=bicprior(),
  )
}
...
We estimate the correlations using the formula from the main text, using the posterior distribution of \phi and a least-squares estimator for $var(x_j|y, x_{-j}, z)$. The results are shown below for the partial correlations of the MSC outcomes from Figure 4 with TV, electronic games (EG), social media (SM), other internet (OI) usage, and having an own computer (OC).

```{r warning=FALSE}
# calc conditional var(x_j|y, x_{-j})
get_cond_var_x = function(xvar, regdata) {
  fit = lm(paste(xvar, "~ ."), regdata)
  n = nrow(regdata); p = ncol(regdata)
  cond_var = sum(residuals(fit)^2) / (n-p)
  return(cond_var)
}

# get conditional cov for one y
get_cond_var_y = function(yvar) {
  b = coef(ms_lin[[yvar]])
  phi = b["phi", "estimate"]
  bx = b[2:(1+length(mcs$x_vars)), "estimate"]
  cond_var = sapply(names(bx), get_cond_var_x, datareg)
  r = -bx*sqrt(phi*cond_var)
  return(r)
}

# combine
corr = t(sapply(names(mcs$y_cont), get_cond_var_y))
colnames(corr) = c("TV", "EG", "SM", "OI", "OC")
